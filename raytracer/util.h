...
struct Material {
		...
		// Material( Colour ambient, Colour diffuse, Colour specular, double exp, float glossy_index, float refraction_index, int flag ) :
		// ambient(ambient), diffuse(diffuse), specular(specular),
		// specular_exp(exp), glossy_index(glossy_index), refraction_index(refraction_index), flag(flag) {}		

		Material( Colour ambient, Colour diffuse, Colour specular, double exp, double glossy,double refraction, int flag ) :
		ambient(ambient), diffuse(diffuse), specular(specular),
		specular_exp(exp), glossy(glossy), refraction(refraction), flag(flag) {}		

	// Material( Colour ambient, Colour diffuse, Colour specular, double exp, int flag ) :
	// 	ambient(ambient), diffuse(diffuse), specular(specular), 
	// 	specular_exp(exp), flag(flag) {}
...
struct Intersection {
	...
	//Added a centerpoint for texture mapping purpose 
	Point3D CenterPoint;
};
