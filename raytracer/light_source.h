...
class LightSource {
public:
	virtual void shade( Ray3D& ,std::vector<Ray3D> incident_rays) = 0;
	virtual Point3D get_position() const = 0; 
	virtual std::vector<Point3D> sampling() = 0;
};
...
public:
	...
	void shade( Ray3D& ray , std::vector<Ray3D> incident_rays);
	Point3D get_position() const { return _pos; }
	std::vector<Point3D> sampling();
